...
def uri_put_file(creds, uri, fp, content_type=None, conn=None):
    ...
    bucket_id = url_tup.netloc
    filename = url_tup.path[1:]  # Strip the leading slash
    f = conn.upload_file(bucket_id, filename, fp)
    return B2Key(f['fileName'], f['contentLength'])
